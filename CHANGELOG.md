...
Standalone CORS fix, branched from 6.1.0 (NOT 6.2.0) — deliberately does not
include the typed-auth-token work (6.2.0/9ab1b1b onward), which is paused
pending its 7.0.0 rework and an independent adversarial review before any
platform adopts it. This lets platforms pick up the CORS fix without also
inheriting the in-progress, not-yet-reviewed auth model.
...
Verified: 616/616 tests pass on the original 6.2.0-based commit (600
pre-existing + 16 new, including a test that a config file calling
`Env::get_instance()` is rejected without ever executing, not merely
discouraged); re-run against this 6.1.0-based branch below. Root cause was
reproduced live against progalaxy-platform (missing
`Access-Control-Allow-Origin` on both the `OPTIONS` preflight and the actual
`POST /api/auth/exchange` for a real browser `Origin: http://localhost:3040`
header) before this fix was written.
