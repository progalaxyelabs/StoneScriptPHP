...
A real production incident (a `create-invoice`-style function whose PHP DTO
was generated by an old, pre-`{Class}Params`-object codegen path and never
regenerated after the SQL function grew 17 optional/DEFAULT'd parameters)
made every call with a legitimately-null optional field throw a PHP
`TypeError` before the database was ever reached — invisible to per-endpoint
unit tests, because they exercise the DTO through its own (wrong) type
contract. Two hardenings close this gap:

**`cli/generate-model.php` (codegen self-defense, 3 layers):**
- Fails loud (exit 1, names the function) on an inline `--` comment INSIDE a
  `CREATE FUNCTION(...)` parameter list — a real, previously-discovered
  footgun (confirmed live in a fleet platform's own SQL, which had to move
  an explanatory comment outside the parens specifically to avoid this) that
  can silently corrupt parameter names/types/nullability.
- Rejects any parsed parameter name that isn't a plain identifier.
- Cross-checks its own nullable/DEFAULT detection with a second, independently
  written regex pass; disagreement fails loud instead of guessing.
- Verifies its own emitted `{Class}Params` property lines against the
  computed spec before writing the file (emission-step self-check).
- **Fixed a genuine, previously-undetected bug found via this work:** a
  DEFAULT'd `json`/`jsonb` parameter (e.g. an items/metadata payload — a
  common pattern) emitted `public ?mixed $x = null;`, which is a PHP fatal
  error (`mixed` is already nullable and cannot be marked `?mixed`). Every
  such function's generated file was completely unusable. Now emits
  `public mixed $x = null;`.

**`php stone validate schema-contract` (new deploy-gate command,
`cli/validate-schema-contract.php`):** connects to a REAL, migrated
PostgreSQL and compares every committed `Fn*.php` DTO's `{Class}Params`
(via reflection) against the function's TRUE live signature read straight
from `pg_proc`/`pg_type` (param count, order, and — using the fact that
Postgres DEFAULT parameters are always trailing among a function's input
args — nullability), independent of what the .pgsql source text says or
whether it's even been migrated. Catches drift introduced AFTER a correct
codegen run (SQL changed, DTO never regenerated) — the class of bug the
codegen-side guards above cannot see, because codegen only runs once. Also
flags legacy pre-`{Class}Params`-object wrappers (no way to express
optionality at all) with an actionable message. `--strict` additionally
checks PHP/SQL type-family agreement. Proven against a real production
function (verified: 17 of 19 real optional parameters correctly flagged) —
FAILs on the stale legacy wrapper, PASSes once regenerated with the fixed
codegen above.
